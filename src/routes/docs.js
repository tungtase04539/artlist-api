...
<script>window.onload=()=>SwaggerUIBundle({url:'/openapi.json',dom_id:'#ui'})</script></body></html>`);
  ...
  // Suno console (中文) — khách nhập API key, tạo nhạc, xem tài liệu API. Canonical: '/suno'.
  ...
  // Bare root '/' → /suno. (Vercel cũng redirect ở edge; đây là fallback nếu function vẫn nhận '/'.)
  app.get('/', async (req, reply) => reply.code(302).header('location', '/suno').send());
